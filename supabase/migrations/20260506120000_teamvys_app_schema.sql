...
create table if not exists public.course_documents (
  ...
  product_id text not null,
  activity_type text not null default 'Kroužek' check (activity_type in ('Kroužek', 'Tábor')),
  kind text not null check (kind in ('gdpr', 'guardian-consent', 'health', 'departure', 'infection-free', 'packing')),
  title text not null,
...
begin
  ...
  alter table public.course_documents add constraint course_documents_activity_type_check check (activity_type in ('Kroužek', 'Tábor'));
  alter table public.course_documents drop constraint if exists course_documents_kind_check;
  alter table public.course_documents add constraint course_documents_kind_check check (kind in ('gdpr', 'guardian-consent', 'health', 'departure', 'infection-free', 'packing'));
end $$;
