-- QR trick second-scan: award half XP when a participant re-scans a trick they already claimed.
-- First scan = full XP (existing flow). Second scan = half XP. Third+ = blocked (already-claimed).

-- Tracking table for rescan bonus claims
create table if not exists public.qr_trick_rescan_claims (
...
create unique index if not exists qr_trick_rescan_claims_participant_trick
  on public.qr_trick_rescan_claims (participant_id, trick_id);

alter table public.qr_trick_rescan_claims enable row level security;
do $$
begin
  if not exists (
    select 1 from pg_policies
    where tablename = 'qr_trick_rescan_claims'
      and policyname = 'qr_trick_rescan_claims all'
  ) then
    create policy "qr_trick_rescan_claims all"
      on public.qr_trick_rescan_claims for all using (true) with check (true);
  end if;
end $$;

-- Rebuild teamvys_claim_qr_trick with half-XP rescan logic.
...
begin
  ...
  if exists (
    ...
    -- Rescan already used → fully blocked
    if exists (
    ...
    -- Second scan → half XP
    half_xp := greatest(1, trick_record.xp / 2);

    insert into public.qr_trick_rescan_claims (participant_id, trick_id, xp_awarded)
    values (participant_record.id, trick_record.id, half_xp)
    on conflict (participant_id, trick_id) do nothing;

    -- Direct XP update (bypasses trigger which would award full trick XP)
    update public.participants
    set xp = coalesce(xp, 0) + half_xp
  ...
  -- First claim — normal insert path
  award_id := case
  ...
  exception when unique_violation then
    -- Race condition: treat as second scan
    if not exists (
      select 1 from public.qr_trick_rescan_claims
      where participant_id = participant_record.id
        and trick_id = trick_record.id
      ...
      values (participant_record.id, trick_record.id, half_xp)
      on conflict (participant_id, trick_id) do nothing;
      update public.participants set xp = coalesce(xp, 0) + half_xp where id = participant_record.id;
      status := 'claimed';
      xp := half_xp;
    else
      status := 'already-claimed';
