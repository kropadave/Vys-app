-- Fix: "column reference xp is ambiguous" in teamvys_claim_qr_trick.
-- The OUT-column named `xp` clashed with `participants.xp` in UPDATE SET clauses.
-- Fix: qualify with participant_record.xp (PL/pgSQL record field — unambiguous).
...
begin
  ...
  if exists (
    ...
    -- Second scan → half XP (use local variable to avoid ambiguity)
    half_xp := greatest(1, trick_record.xp / 2);
    out_xp  := coalesce(participant_record.xp, 0) + half_xp;
    ...
    -- Qualify table alias to avoid ambiguity with OUT param named `xp`
  ...
  exception when unique_violation then
    ...
    if not exists (
      ...
      values (participant_record.id, trick_record.id, half_xp)
      on conflict (participant_id, trick_id) do nothing;
      update public.participants set xp = out_xp where id = participant_record.id;
      status := 'claimed';
      xp := half_xp;
    else
      status := 'already-claimed';
